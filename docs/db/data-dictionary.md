...
- permissions: Admin-granted access in the team portal beyond what the person's own data implies; `revenue` opens the Revenue section. Set from the admin Team roster, never by the employee. Valid values: [revenue].
...
Columns:
- preferred_weekday: The weekday the member prefers for 1-1s, 0 Sunday to 6 Saturday; the cycle lands rolled dates on it. Null means no preference (K.34).
...
- proposed_one_on_one_on: A date proposed for the next 1-1 and not yet confirmed; cleared when the coach confirms it onto next_one_on_one_on or declines it. Null means no open proposal (K.32).
...
- missed_at: When the daily coaching pass found this scheduled 1-1 still sitting on a date that had passed; null on a 1-1 that was never missed, cleared by a move and kept when it is marked held late (K.36).
