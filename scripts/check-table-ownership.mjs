...
// pair. `--explain <pair>` lists the file → table edges behind one pair.
//
...
/**
 * Every `.from(...)` table access in a source string: [{ table, write }]. A
 * `.from(NAME)` resolves through a file-level const; one that does not resolve
 * is reported as `{ table: null, name, write: true }` when the chain writes,
 * and skipped when it only reads.
 */
...
export function collectAccesses(root, manifest = loadManifest(root)) {
  ...
  for (const rel of listSourceFiles(root)) {
    ...
    for (const { table, name, write } of tableAccesses(source)) {
      if (table === null) {
        edges.push({ file: rel, from, table: null, name, to: "unknown", write });
        continue;
      }
