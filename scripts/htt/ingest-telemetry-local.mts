...
// exclusion, the session store behind the hours rule (entities/htt/day-hours.ts)
// and the same idempotency keys, so running it twice, or from two machines,
// never double counts. Legacy `record_type: "human"` records are ignored: days
// are recomputed on the server from sessions.
//
...
async function writeTokens(body: NonNullable<ReturnType<typeof buildEndBody>>, personId: string | null) {
  const common = {
    ...
    person_id: personId,
    source: body.source,
    // NOT NULL in htt.token_entries; the body builders leave it optional, and
    // this script has always forwarded whatever they produced.
    occurred_at: body.occurred_at as string,
    occurred_on: null,
    status: "recorded",
  };
  const sessionId = body.session_id;
  const rows: Array<Database["htt"]["Tables"]["token_entries"]["Insert"]> = [];
...
// 4) Apply.
let linked = 0, skippedRepo = 0, rejected = 0, skippedNoKey = 0, excluded = 0, rows = 0, legacyHuman = 0;
...
for (const e of entries) {
  ...
  if (e.record_type === "human") { legacyHuman++; continue; }
  ...
  if (!dryRun) {
    const days = await upsertWorkSession({
      ...
      // Typed lines are what the hours rule prices; without them the row falls
      // back to the legacy all-lines clock and every later rule change misses it.
...
let relinked = 0, daysRecomputed = 0;
if (!dryRun) {
  for (const [personId, days] of daysByPerson) daysRecomputed += await recomputePersonDays(personId, [...days]);
...
console.log(JSON.stringify({ entries: entries.length, linked, rowsWritten: rows, daysRecomputed, legacyHuman, rejected, excluded, skippedNoKey, skippedRepo, relinked, unattributedPerson: [...personCache.values()].filter((v) => v === null).length }, null, 1));
