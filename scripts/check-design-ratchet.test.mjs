...
describe("check-design-ratchet gate", () => {
  ...
  it("fails when a file absent from the baseline has any inline style", () => {
    ...
    fs.writeFileSync(path.join(root, "components/New.tsx"), TWO_STYLES);
    expect(checkDesignRatchet(root).violations).toEqual([
      "components/New.tsx: 2 inline style(s) in a file with no baseline entry (new files start at 0)",
    ]);
  });
