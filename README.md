...
To make the choice impossible to get wrong, **these two files are the only
`.sql` files in this repo**. `docs/db/` carries the data dictionary
(`data-dictionary.md`) and nothing executable. If `find . -name '*.sql'` shows
anything other than the two under `supabase/`, you are not looking at a clean
copy of this repo.
...
no spaces — `apa`, `arca-wellness`, `b-store`), **their production domain**, and
...
| Variable | Replaces |
|---|---|
| `NEXT_PUBLIC_SITE_URL` | `https://www.edge8.ai` as the canonical origin for links, sitemaps and emails |
| `ADMIN_EMAILS` | the upstream admin inbox that contact-form submissions fall back to |
| `EMAIL_FROM` | `Edge8 <notifications@edge8.ai>` as the transactional sender |
| `MARKETING_EMAIL_FROM` | `Edge8 <hello@edge8.ai>` as the marketing sender |
...
| `kernel/config/utils.ts` | `SOURCE_SITE = "edge8.ai"` → the client's domain. Stamped on every inquiry row as its source |
...
values ('<id from above>', '<one line on what the client does>', '<who they sell to>');
```
