...
import { RevenueAccessToggle } from "./RevenueAccessToggle";
...
import {
  selectTimeOff,
  LEAVE_TYPE_LABEL,
  countWorkingDays, listHolidayDatesCovering,
  formatDays, formatNumber, getMemberLeave, hoursToDays, statusTone as leaveStatusTone,
...
export default async function TeamMemberPage({ params }: { params: { id: string } }) {
  ...
  // Wages/PII are gated to the finance-cleared admins. A plain admin never has the
  ...
  const [surveyResponses, assignments, assignableCompanies, avatarRes, sensitive, signedInIds, cycleRes, salaryHistory, custody, reviewHistory, probationDone, leaveById] =
    await Promise.all([
      ...
      companyOs
        .from("team_members")
        .select("employment_stage, probation_ends_on, contract_start_date, permissions")
        .eq("id", params.id)
        .maybeSingle(),
  ...
  const leaveHolidays = await listHolidayDatesCovering(requests.flatMap((r) => [r.start_date, r.end_date]));
  ...
  // Balance from the leave policy rules, the same figure History and /team show.
  const leave = leaveById.get(m.id) ?? null;
  const balance = leave?.balance ?? null;
  const leaveDays = (h: number) => formatNumber(hoursToDays(h, leave?.policy?.accrual.hoursPerDay ?? 8));
    ...
    <>
      ...
      {balance && (
        <div className="admin-kpi-grid u-mb-5">
          <MetricCard
            label="Accrued"
            value={leaveDays(balance.accruedHours + balance.anchorHours + balance.adjustedHours)}
            sub="days so far this policy year"
          />
          <MetricCard label="Used" value={leaveDays(balance.usedHours)} sub="days taken" />
          <MetricCard label="Remaining" value={leaveDays(balance.remainingHours)} sub="days left" />
        </div>
      )}
          ...
          <div className="admin-card admin-section-card">
            ...
            <dl className="admin-kv">
              ...
              <dd>
                ...
                />
              </dd><RevenueAccessToggle teamMemberId={m.id} permissions={(cycleRes.data as { permissions?: string[] } | null)?.permissions} />
              {m.end_date && (
                <>
                  <dt>End date</dt>
        ...
        <div>
          ...
          {m.person_id && !canSeePII && (
            <div className="admin-card admin-section-card">
              ...
              <p className="admin-cell-muted">Restricted — visible to finance-cleared admins only.</p>
            </div>
          )}
        </div>
      </div>
