...
export async function getPersonSurveyResponses(personId: string): Promise<PersonSurveyResponse[]> {
  const { data, error: responsesErr } = await selectSurveyResponses("id, survey_id, submitted_at, created_at, surveys!survey_id(name)")
    .eq("person_id", personId)
    .order("submitted_at", { ascending: false, nullsFirst: false });
  if (responsesErr) console.error("[company-os/surveys] survey_responses", responsesErr);
  ...
  const [fieldsRes, answersRes] = await Promise.all([
    ...
    selectSurveyAnswers("response_id, field_id, value").in("response_id", responseIds),
  ]);
...
// Each response carries its questions and answers, with sensitive answers
// flagged, so the company page opens one in the same drawer as the Team member
// profile. A company links a handful of people, so one read per person is fine.
export async function getSurveyResponsesForCompany(companyId: string): Promise<CompanySurveyResponse[]> {
  const { data: links, error: linksErr } = await selectPersonCompanies("people:people!person_id(id, full_name, email)")
    .eq("company_id", companyId);
  if (linksErr) console.error("[company-os/surveys] person_companies", linksErr);
  ...
  const perPerson = await Promise.all(
    [...nameById].map(async ([personId, respondentName]) =>
      (await getPersonSurveyResponses(personId)).map((r) => ({ ...r, respondentName })),
    ),
  );
  return perPerson.flat().sort((a, b) => b.submittedAt.localeCompare(a.submittedAt));
}
