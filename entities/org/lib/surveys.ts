...
export async function getSurveyResponsesForCompany(companyId: string): Promise<CompanySurveyResponse[]> {
  ...
  const { data, error: companyResponsesErr } = await selectSurveyResponses("id, person_id, submitted_at, created_at, surveys!survey_id(name)")
    .in("person_id", personIds)
    .order("submitted_at", { ascending: false, nullsFirst: false });
  if (companyResponsesErr) console.error("[company-os/surveys] survey_responses", companyResponsesErr);
