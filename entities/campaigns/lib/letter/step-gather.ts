...
export const letterGatherOutput = z.object({
  points: z.array(z.object({
    date: z.string().describe("YYYY-MM-DD the fact belongs to."),
    fact: z.string().describe("One or two plain sentences, first person, specific: places, numbers, what happened. No client or client-company names; describe people by role and place."),
    source: z.string().describe("Which source item it came from (its title)."),
  })).describe("Five to eight data points, most recent first."),
});
...
You are preparing material for Dave's weekly letter. From the raw material, pull out the five to eight things that actually happened in the last ten days that a reader would find real and specific: where he was, who he was in a room with (by role, never by name), what was built, what surprised him, what number moved. Prefer the most recent. Each point is one or two first-person sentences a letter could use verbatim. Never name a client or a client's company. Never invent.`;
  ...
  const r = await callWriterModel({ step: "letter-gather", system, user, schema: letterGatherOutput });
  if (!r.ok) return r;
