...
export async function loadLetter(id: string): Promise<Loaded<Letter>> {
  const { data, error } = await companyOs.from("email_campaigns").select("id, name, subject, preheader, body_md, blocks, brand_id, status, from_email, reply_to, agent_step, agent_error, agent_started_at, agent_notes")
    .eq("id", id)
    .maybeSingle();
