...
const MODEL = modelFor("brand-writer", "frontier");
  ...
  properties: {
    outputs: {
      ...
      items: {
        ...
        properties: {
          channel: { type: "string", enum: ["email", "linkedin", "facebook", "twitter", "blog"] },
          ...
          social_style: { type: "string", description: "LinkedIn/Facebook/Twitter only: a slug from the brand's preferred social styles." },
          ...
          image_brief_md: { type: "string", description: "A short image brief: hero concept, palette, and ratios, following the brand's image style. Where the style carries words (a typographic splash, a data diagram, a concept card), give the exact headline or figure and label to set, a handful of words, spelled out in quotes." },
        },
      },
    },
  },
...
${input.plan ? `# Campaign plan (decided before drafting; follow it)
${input.plan}
...
` : ""}${input.history ? `# Recent posts (newest first)
${input.history}
