...
const SCHEMA = {
  ...
  required: ["faq", "primary_keyword", "title", "title_tag", "meta_description", "slug", "secondary_keywords", "excerpt", "category"],
  properties: {
    faq: {
      type: "array",
      ...
      items: {
        ...
        properties: {
          ...
          answer: { type: "string", description: "Two to four sentences, plain prose, self-contained, no Markdown. Backed by data: at least one specific figure the post body states, with its source named when it has one." },
        },
      },
    },
    ...
    primary_keyword: { type: "string", description: "The phrase a real person would search, taken from inside the first FAQ question exactly as written there. Not a sentence." },
...
${taken.length ? taken.map((r) => `- ${r.primaryKeyword ?? "(no keyword)"} / ${r.primaryQuestion ?? "(no question)"}`).join("\n") : "(none)"}`;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const r = await callWriterModel<SeoOut>({ step: "seo", system, user: user + feedback, schema: SCHEMA });
    if (!r.ok) return r;
    out = r.data;
    failures = [];
