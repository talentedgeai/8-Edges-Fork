...
// brand_contacts is contacts' table; its door hands back the builder so the
// paging filters below stay here.
import { selectBrandContacts } from "@/entities/contacts";
...
export type BroadcastSegment = { personas?: string[]; sendWindow?: { weekday: number; hour: number } };
...
const CAMPAIGN_SELECT =
  "id, name, subject, preheader, body_md, blocks, status, segment, from_email, reply_to, batch_size, brand_id, scheduled_at, approved_at, approved_by, sent_at, created_by, created_at, brands(name)";
...
export async function listBroadcasts(): Promise<{ rows: BroadcastRow[]; error?: string }> {
  const { data, error } = await companyOs.from("email_campaigns").select(CAMPAIGN_SELECT).order("created_at", { ascending: false });
