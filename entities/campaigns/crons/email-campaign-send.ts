...
import { checkSendGate } from "@/entities/campaigns/lib/broadcasts";
...
async function handler(_req: Request) {
  ...
  for (const row of rows) {
    ...
    if (gate.verdict === "suppress") {
      const { error: skipError } = await companyOs.from("email_campaign_recipients").update({ status: "skipped", skip_reason: gate.reason })
        .eq("id", row.id);
      if (skipError) {
        writeFailures += 1;
