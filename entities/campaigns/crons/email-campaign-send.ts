...
async function handler(_req: Request) {
  ...
  if (personal && !learners) {
    ...
    return NextResponse.json({ campaign: campaign.id, deferred: rows.length, error: "learning progress unavailable" }, { status: 500 });
  }
  ...
  for (const row of rows) {
    ...
    // the one-email-a-day cap across both kinds. Asked through the module the
    // personal path uses, so the two cannot drift again (A.15): this tick never
    // used to ask about the cap at all, which is how a personal message at
    // 08:20 failed to stop this batch at 08:30.
    const decision = await decideSend("broadcast", { personId: row.person_id, email: row.email }, new Date(nowIso));

    if (decision.action === "defer") {
      // Put the row back so the next tick retries it, rather than marking
      // someone permanently skipped over a transient timeout or a day's cap.
      const { error: deferError } = await companyOs.from("email_campaign_recipients").update({ status: "pending", claimed_at: null })
        .eq("id", row.id);
      if (deferError) {
        writeFailures += 1;
    ...
    if (decision.action === "skip") {
      const { error: skipError } = await companyOs.from("email_campaign_recipients").update({ status: "skipped", skip_reason: decision.reason })
        .eq("id", row.id);
      if (skipError) {
        writeFailures += 1;
    ...
    if (result.ok) {
      const { error: sentError } = await companyOs.from("email_campaign_recipients").update({
          status: "sent",
      ...
      const { error: failError } = await companyOs.from("email_campaign_recipients").update({ status: "failed", error: result.error })
        .eq("id", row.id);
      if (failError) {
        writeFailures += 1;
