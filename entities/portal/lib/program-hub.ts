...
export async function listHubBoardsForActor(actor: PortalActor): Promise<PortalHubBoard[]> {
  if (actor.companyScope.length === 0) return [];
  const { data, error: boardsError } = await companyOs.from("boards").select("id, name, slug, ai_program_id").in("client_company_id", actor.companyScope).eq("status", "active").is("archived_at", null).order("sort_order", { ascending: true });
