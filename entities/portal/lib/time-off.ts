...
export async function getAssignedTimeOff(actor: PortalActor): Promise<PortalTimeOffEntry[]> {
  ...
  const { data, error: timeOffError } = await companyOs
    .from("time_off")
  ...
  const { data: directoryRows, error: directoryRowsError } = await companyOs
    .from("team_directory")
    .select("id, full_name")
    .in("id", memberIds);
...
export async function getAssignedLeaveDirectory(actor: PortalActor): Promise<PortalLeaveDirectoryRow[]> {
  ...
  const { data, error: teamDirectoryError } = await companyOs
    .from("team_directory")
  ...
  const { data: assignmentRows, error: assignmentRowsError } = await companyOs
...
export async function getLeaveDecisionQueue(actor: PortalActor): Promise<PortalDecisionRequest[]> {
  ...
  const { data, error: timeOffError2 } = await companyOs
    .from("time_off")
  ...
  const { data: directoryRows, error: directoryRowsError2 } = await companyOs
    .from("team_directory")
    .select("id, full_name")
    .in("id", managedIds);
