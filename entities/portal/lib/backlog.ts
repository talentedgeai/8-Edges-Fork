...
import { selectClientRoadmapGroups, selectAiPrograms, insertClientBacklogItems, updateClientBacklogItems } from "@/entities/client-programs";
...
export async function proposeItemForActor(
  ...
  if (aiProgramId) {
    const { data: programRow, error: programRowError } = await selectAiPrograms("id")
      .eq("id", aiProgramId)
