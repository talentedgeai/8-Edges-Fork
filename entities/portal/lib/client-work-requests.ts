...
import { insertInquiries, workRequestPath } from "@/entities/company-os";
...
export async function listPortalInquiriesForActor(
  ...
  const { data, error: inquiriesError } = await companyOs.from("inquiries").select("id, subject, status, created_at").eq("person_id", actor.personId).eq("source", "portal").order("created_at", { ascending: false }).limit(5);
