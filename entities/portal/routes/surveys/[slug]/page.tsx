...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const { data, error: metaErr } = await companyOs
    .from("surveys")
...
export default async function PublicSurveyPage({
  ...
  params: { slug: string };
  searchParams?: { cohort?: string; review?: string; t?: string };
}) {
  const { data, error: surveyErr } = await companyOs
    .from("surveys")
    .select(
  ...
  if (survey.purpose === "performance_review") {
    ...
    const { data: reviewFieldsData, error: reviewFieldsErr } = await companyOs
      .from("survey_fields")
