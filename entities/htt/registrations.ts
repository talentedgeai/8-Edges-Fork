...
import { htt, companyOs } from "@/kernel/data/supabase";
import { GitHubHttpError, isNoAccessError, type GitHubClient } from "./github";
import { slugify } from "@/kernel/config/slug";
import { insertAiPrograms } from "@/entities/portal";
import { ensureProgramBoard, insertCompanies, updateCompanies } from "@/entities/company-os";
import { insertPersonGitEmails } from "@/entities/team";
...
export async function applyRegistration(
  ...
  let companyId: string;
  {
    const { data: existing, error: existingError } = await companyOs
      ...
      .maybeSingle();
    if (existingError) console.error("[htt] companies read failed:", existingError.message);
    if (existing) {
      companyId = existing.id;
      notes.push(`company exists: ${req.client.name}`);
      if (!existing.is_ai_program) {
        await updateCompanies({ is_ai_program: true }).eq("id", companyId);
        notes.push("company flagged is_ai_program");
      }
    } else {
      const { data: inserted, error } = await insertCompanies({ name: req.client.name, is_ai_program: true })
        .select("id")
        .single();
  ...
  let repoId: string;
  {
    const { data: existing, error: existingError2 } = await htt
      .from("repos")
      .select("id")
      .eq("github_repo", req.repo_full_name)
      .maybeSingle();
    if (existingError2) console.error("[htt] repos read failed:", existingError2.message);
    if (existing) {
      notes.push(`repo already registered for ${req.repo_full_name}: no-op`);
      return { status: "noop", notes };
    }

    const { data: program, error: progErr } = await insertAiPrograms({
    ...
    // Every program gets a workboard by default; a board failure is a WARN,
    // not a reason to leave the repo unregistered and retry it nightly.
    const board = await ensureProgramBoard({ id: program.id, name: req.project_name, companyId });
    notes.push(board.ok ? `board created: ${board.slug}` : `WARN: default board failed: ${board.error}`);
  ...
  if (req.type === "client" && Array.isArray(req.exclude_identities)) {
    for (const identity of req.exclude_identities) {
      const { data: existing, error: existingError3 } = await htt
        .from("client_identities")
        .select("id")
        .eq("repo_id", repoId)
        .ilike("git_email", identity.git_email ?? "")
        .maybeSingle();
      if (existingError3) console.error("[htt] client_identities read failed:", existingError3.message);
      if (!existing) {
  ...
  if (committerEmail) {
    const { data: resolved, error: resolvedError } = await htt.rpc("resolve_contributor", { p_email: committerEmail });
    if (resolvedError) console.error("[htt] resolve_contributor rpc read failed:", resolvedError.message);
    if (resolved) {
      const { data: aliasExists, error: aliasExistsError } = await companyOs
        .from("person_git_emails")
        .select("id")
        .eq("git_email", committerEmail)
        .maybeSingle();
      if (aliasExistsError) console.error("[htt] person_git_emails read failed:", aliasExistsError.message);
      if (!aliasExists) {
        const { error } = await insertPersonGitEmails({
