...
export function getCentralEmail(): string {
  return process.env.CENTRAL_EMAIL ?? "derek.nguyen@edge8.ai";
}
...
/**
 * Read a repo's committed `.claude/project.json` and return the parsed value.
 * Never throws: an unusable "owner/name" string, a missing file, no access and
 * a transport failure all yield null, because the ingest crons treat an
 * unreadable repo as "nothing to ingest". A file that is present but not valid
 * JSON is logged, since that is a real defect in the repo rather than an
 * expected absence. Callers pick the field they need off the parsed value.
 */
export async function readProjectJson(client: GitHubClient, repo: string): Promise<unknown | null> {
  const [owner, name] = repo.split("/");
  if (!owner || !name) return null;
  let file: RepoFile | null;
  try {
    file = await fetchRepoFile(client, owner, name, ".claude/project.json");
  } catch {
    return null;
  }
  if (!file) return null;
  try {
    ...
    console.error(
      `[htt] ${repo}: .claude/project.json is not valid JSON`,
