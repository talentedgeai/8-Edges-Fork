...
import { parseSessionTrailer, trailerIsSane } from "./session-trailer";
...
export async function upsertPRs(repoId: string, prs: FetchedPR[]): Promise<UpsertResult> {
  ...
  for (const pr of prs) {
    ...
    // Session trailer (human-token-tracker docs/plans/2026-09-01-session-start-trailer.md):
    // the PR body carries the session UUID + start on machines where telemetry can't
    // deliver. Always written (null when absent or insane) so a body edit that removes
    // the trailer clears the columns on the next sync — the upsert stays idempotent.
    const trailer = parseSessionTrailer(pr.body);
    const trailerOk = trailer != null && trailerIsSane(trailer.startedAt, pr.openedAt);
    rows.push({
      repo_id: repoId,
      ...
      session_id: trailerOk ? trailer.sessionId : null,
      session_started_at: trailerOk ? trailer.startedAt : null,
      session_start_source: trailerOk ? "commit-trailer" : null,
    });
  }
