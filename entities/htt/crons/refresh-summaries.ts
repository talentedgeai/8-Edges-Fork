...
async function regenerateOne(req: Request) {
  ...
  const { data: repoRow, error: repoErr } = await htt
    .from("repos")
    .select("id, github_repo")
    .eq("github_repo", body.repo)
    .maybeSingle();
  // A read failure is not an unknown repo — reporting 404 for one would send the
  // caller looking for a registration problem that does not exist.
  if (repoErr) {
    console.error("[htt] repos lookup failed for", body.repo, repoErr.message);
    return NextResponse.json({ error: "repo lookup failed" }, { status: 500 });
  }
