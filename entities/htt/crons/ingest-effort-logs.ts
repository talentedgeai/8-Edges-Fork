...
import { createGitHubClient, readProjectJson, type GitHubClient } from "@/entities/htt/github";
import { ingestEffortLogs, type EffortLogRepo } from "@/entities/htt/ingest-effort-logs";
import type { EffortLogEntry, TokenEntryBody } from "@/entities/htt/effort-log-ingest";
import { buildOwnerEmailSet, isOwnerEmail, type ClientIdentityRow } from "@/entities/htt/contributor-kind";
...
async function readRepoEffortLog(gh: GitHubClient, repo: string): Promise<EffortLogEntry[]> {
  const json = (await readProjectJson(gh, repo)) as { effort_log?: unknown } | null;
...
async function handler(req: Request) {
  ...
  const { data: identityRows, error: identityRowsError } = await htt
    .from("client_identities")
    .select("repo_id, github_login, git_email");
  if (identityRowsError) console.error("[htt] client_identities read failed:", identityRowsError.message);
...
export const GET = (req: Request) => withRoutineRun("/api/cron/htt-ingest-effort-logs/", req, handler);
