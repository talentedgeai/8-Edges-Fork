...
async function handler(req: Request) {
  ...
  const { data: run, error: runErr } = await htt
    .from("sync_runs")
    .insert({ started_at: new Date().toISOString() })
    .select()
    .single();
  // The sync_runs row is this run's only record of itself: without it nothing
  // downstream can see that a sync happened, what it touched, or that it failed.
  // Abort rather than sync unobservably.
  if (runErr || !run) {
    console.error("[htt] sync_runs insert failed:", runErr?.message ?? "no row returned");
