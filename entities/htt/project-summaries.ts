...
type ExecutiveDecision = "skip" | "generate";
...
export async function refreshRepoSummaries(
  ...
  const { data, error: projectSummariesError } = await htt
    .from("project_summaries")
    .select("kind, content, as_of, source_key")
    .eq("repo_id", repo.id);
  if (projectSummariesError) console.error("[htt] project_summaries read failed:", projectSummariesError.message);
...
export async function recentDigestPrs(repoId: string): Promise<DigestPr[]> {
  ...
  if (pullRequestsError) console.error("[htt] pull_requests read failed:", pullRequestsError.message);
