...
export async function downloadFresh(path: string): Promise<string | null> {
  ...
  if (!res.ok) {
    console.error(`[library] download ${path} failed: HTTP ${res.status}`, await res.text().catch(() => ''))
    return null
  }
  return res.text()
}
...
async function listObjects(): Promise<{ name: string }[]> {
  ...
  if (!res.ok) {
    console.error(`[library] list ${DOCS_BUCKET} failed: HTTP ${res.status}`, await res.text().catch(() => ''))
    return []
  }
