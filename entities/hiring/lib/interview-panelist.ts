...
// The runtime mirror of PANELIST_SCHEMA. Structured output is a request, not a
// guarantee, so the model's JSON is parsed through this before any scorecard or
// score row is written.
const panelistOutput = z.object({
  ...
  next_round_questions: z.array(z.string()),
  summary: z.string(),
});
...
async function runPanelist(interviewId: string): Promise<Ok | Err> {
  ...
  let rawOut: unknown;
  try {
    rawOut = JSON.parse(out.text);
  } catch (e) {
    return { ok: false, error: `The model returned invalid JSON: ${e instanceof Error ? e.message : String(e)}` };
  }
  const validated = panelistOutput.safeParse(rawOut);
  if (!validated.success) return { ok: false, error: zodIssuesToMessage(validated.error.issues) };
  return writeScorecard(interviewId, validated.data);
}
