...
import { generatePrepAction, logOneOnOne, scheduleOneOnOne } from "@/entities/coaching/lib/meeting-actions";
...
export function CoachHeaderActions({
  ...
  leadWithSchedule = false,
}: {
  profileId: string;
  nextMeetingId: string | null;
  nextHasPrep: boolean;
  // True when nothing has happened between these two yet, so this button is
  // the page's one filled action rather than one outline among three.
  leadWithSchedule?: boolean;
}) {
  ...
  return (
    <div className="admin-coach-hero__actions">
      <div className="admin-coach-hero__actionbtns">
        ...
        {/* One filled control, picked by state (L.13, the rule K.57 gave the
            roster rows). With a 1-1 booked, the prep is the job and it leads;
            with nothing behind the two of them, booking the first one is. */}
        <button
          className={`admin-btn${leadWithSchedule ? " admin-btn--growth" : ""}`}
          disabled={busy}
          onClick={() => toggle("schedule")}
        >
          {panel === "schedule" ? "Cancel" : leadWithSchedule ? "Schedule the first 1-1" : "Schedule 1-1"}
        </button>
