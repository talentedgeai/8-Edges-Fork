...
export async function addTalkingPoint(profileId: string, body: string): Promise<Result> {
  const actor = await requireTeamMember();
  const res = await coachAddTalkingPoint(actor, profileId, body);
  if (res.ok) {
    refresh(profileId);
...
export async function deleteTalkingPoint(id: string): Promise<Result> {
  const actor = await requireTeamMember();
  const res = await coachDeleteTalkingPoint(actor, id);
  if (res.ok) {
    refresh(res.profileId);
    revalidatePath("/team/my-coaching");
  }
  return res.ok ? { ok: true } : res;
}
...
export async function setOneOnOnesPaused(profileId: string, paused: boolean): Promise<Result> {
  const actor = await requireTeamMember();
  const res = await coachSetOneOnOnesPaused(actor, profileId, paused);
  if (res.ok) refresh(profileId);
  return res;
}
