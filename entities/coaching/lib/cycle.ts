...
//   2) Mid-cycle check-in: halfway through the cycle with open commitments and
//      no check-in since the last 1-1 -> AI-written nudge to the member,
//      recorded on coaching_checkins ("one per cycle" = the idempotence).
//   3) Monthly trends: first days of the month -> prior month's report per
...
import { generateCheckinMessage, generatePrep, generateTrendReport, summarizeMeeting } from "@/entities/coaching/lib/ai";
...
import { rollForward } from "@/entities/coaching/lib/cadence";
...
async function loadActiveProfiles(): Promise<ProfileRow[]> {
  const { data, error: profilesError } = await companyOs
    ...
    .select(
      "id, coach_id, cadence_days, next_one_on_one_on, one_on_ones_paused_at, " +
...
async function syncMinutesForProfile(
  ...
  const { data, error: minutesError } = await companyOs.from("coaching_one_on_ones").select("id, held_on, minutes_token, transcript, meeting_id, summary_markdown").eq("coaching_profile_id", p.id).is("archived_at", null).not("minutes_token", "is", null);
...
export async function runCoachingCycle(todayISO: string): Promise<CoachingRunSummary> {
  ...
  for (const p of profiles) {
    ...
    const rolled = p.paused ? null : rollForward(next, lastHeld, p.cadence_days, todayISO);
    ...
    // 2) Mid-cycle check-in: halfway through the cycle, open commitments, and
    ...
    if (lastHeld && p.memberEmail) {
      ...
      if (d >= half && d < p.cadence_days) {
        const { count: openCount, error: openError } = await companyOs
          .from("coaching_commitments")
          .select("id", { count: "exact", head: true })
          .eq("coaching_profile_id", p.id)
          .in("status", ["open", "on_track", "needs_attention", "blocked"]);
        if (openError) console.error("[team/coaching-cycle] coaching_commitments", openError);
        const { data: recent, error: recentError } = await companyOs
          ...
          .limit(1);
        if (recentError) console.error("[team/coaching-cycle] coaching_checkins", recentError);
