...
type PersonRow = { id: string; display_name: string | null; full_name: string | null; email: string; is_team_member: boolean | null };
...
export async function getWorkboard(query: WorkboardQuery): Promise<WorkboardData> {
  ...
  const [peopleRes, backlogLabelRes, clientBacklogRes, roadmapGroupsRes, logsRes, commentsRes, archivedRes] =
    await Promise.all([
      personIds.length
        ? companyOs.from("people").select("id, display_name, full_name, email, is_team_member").in("id", personIds)
