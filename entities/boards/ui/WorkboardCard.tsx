...
import {
  ...
  epicColorIndex,
  initials,
} from "@/entities/boards/lib/types";
  ...
  return (
    <>
      ...
      <div className="admin-kanban-card-meta">
        ...
        {showBoard && (board?.client_name ? <Badge tone="info" colorIndex={board.client_color}>{board.client_name}</Badge> : <Badge tone="neutral">Internal</Badge>)}
      </div>
      <div className="admin-kanban-card-meta">
        {c.assignee_name ? (
      ...
      {c.status === "done" && c.completed_at && (
        <div className="admin-kanban-card-sub u-ok u-mt-1">✓ Completed {formatDate(c.completed_at)}</div>
      )}
    </>
  );
}
