      ...
      {form && (
        <fieldset className="admin-form" disabled={readOnly}>
          ...
          <CardTargetFields
            ...
            completedAt={activeCard?.status === "done" ? activeCard.completed_at : null}
            readOnly={readOnly}
          ...
          <div className="admin-field">
            ...
            <select className="admin-select" value={form.assigneeId} onChange={(e) => setForm({ ...form, assigneeId: e.target.value })}>
              ...
              {/* The Team list is staff only, so an assignee from outside it would
                  otherwise vanish from the select and read as Unassigned. */}
              {form.assigneeId && !data.people.some((p) => p.id === form.assigneeId) && (
                <option value={form.assigneeId}>{activeCard?.assignee_name ?? "Current assignee"}</option>
              )}
              {data.people.map((o) => (
