...
import { AGING_DAYS } from "@/entities/boards/lib/types";
...
// The one toolbar: the filters (client, then board where a client in view has
// several, assignee, status, then a single board's sprint and epic, or across
// boards the sprint week), the board
// chips, then on the right the view
...
export function WorkboardToolbar({
  ...
  return (
    <div className="admin-toolbar u-mb-3">
      ...
      {f.boardOptions.length > 0 && (
        <MultiSelect label="Filter by board" noun="boards" options={f.boardOptions} value={f.boardFilter} onChange={f.setBoardFilter} />
      )}
      <MultiSelect
      ...
      {!single && f.weeks.length > 0 && (
        <select
          className={`admin-select admin-input--w-sm${f.weekFilter !== "all" ? " is-filtering" : ""}`}
          value={f.weekFilter}
          onChange={(e) => f.setWeekFilter(e.target.value)}
          aria-label="Filter by sprint week"
        >
      ...
      {single && extras && (
        <>
          ...
          <Link className="admin-btn admin-btn--sm" href={`${boardBase}/epics`}>
