...
import { applyUsdConversion, HANDOFF_REJECT_REASONS, LOST_REASONS, reopenLeadAsConnected } from "./deal-helpers";
import { forecastInputsError, loadStageContext, recordDealStageMove, stageEntryPatch } from "@/entities/crm/lib/deal-stage";
...
export async function moveDealStage(
  ...
  const { data: stage, error: stageErr } = await companyOs.from("pipeline_stages").select("name, is_won, is_lost, default_probability")
    .eq("id", toStageId)
    .maybeSingle();
  ...
  // Where the deal is now (for the log) and whether it may enter the stage
  // (Proposal and later need the forecast inputs; lib/deal-stage.ts).
  ...
  // manual override sticks (lib/deal-stage.ts).
  ...
  await recordDealStageMove({ dealId, fromStageId: ctx.deal.stageId, toStageId, movedBy: admin.email });
