...
vi.mock("@/kernel/identity/revenue-access", () => ({
  requireRevenueAccess: async () => ({ email: "admin@example.com" }),
}));
...
vi.mock("@/entities/crm/lib/deal-stage", () => ({
  loadStageContext: async () => ({ ok: true, deal: { stageId: "s-open", amount_cents: 1000, expected_close_date: "2026-10-01" }, stages: [] }),
