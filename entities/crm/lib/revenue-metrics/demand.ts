...
import { selectDeals, selectInquiries, selectMeetings } from "@/entities/crm/lib/reads";
...
export type DemandDeal = { id: string; source: string | null; campaign_id: string | null; status: string | null; amount_usd_cents: number | null; amount_cents: number | null; currency?: string | null; archived_at: string | null };
...
export async function loadDemand(range: Range = DEFAULT_RANGE, now = new Date()): Promise<DemandMetrics> {
  ...
  const [inqRes, dealsRes, campaigns, mtgRes, touchRes] = await Promise.all([
    ...
    selectDeals("id, source, campaign_id, status, amount_usd_cents, amount_cents, currency, archived_at").limit(5000),
    listCampaignOptions(),
