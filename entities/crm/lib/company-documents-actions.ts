...
export async function adminSignedDocumentUpload(input: {
  ...
  if (input.programId) {
    const { data, error: programErr } = await selectAiPrograms("id")
...
export async function adminAddLink(input: {
  ...
  const admin = await requireRevenueAccess();
  if (input.programId) {
    const { data, error: linkProgramErr } = await selectAiPrograms("id")
      .eq("id", input.programId)
      .eq("company_id", input.companyId)
      .maybeSingle();
    if (linkProgramErr) return { ok: false, error: linkProgramErr.message };
    if (!data) return { ok: false, error: "That program does not belong to this company." };
  }
