 ...
 *   blue, blue-bright, mint and dark come from kernel/config/palette.json
 */
...
// Brand hex comes from kernel/config/palette.json, the one file (with tokens.css) allowed to hold it.
const PALETTE = require('../../../kernel/config/palette.json')
const COLORS = {
  blue: PALETTE.blue,
  blueBright: PALETTE.blueBright,
  mint: PALETTE.mint,
  dark: PALETTE.dark,
  white: PALETTE.white,
}

// Satori cannot read color-mix(), so translucent brand layers are built from the palette hex.
const alpha = (hex, a) => {
  const n = parseInt(hex.slice(1), 16)
  return `rgba(${(n >> 16) & 255}, ${(n >> 8) & 255}, ${n & 255}, ${a})` // colour-ok: derived from palette.json, Satori needs rgba()
}
...
function buildTree(card, photoSrc) {
  ...
  const background = isPhoto
    ? [
        ...
        h('div', { key: 'scrim', style: { position: 'absolute', top: 0, left: 0, width: 1200, height: 630, background: 'linear-gradient(90deg, ' + alpha(COLORS.dark, 0.92) + ' 0%, ' + alpha(COLORS.dark, 0.80) + ' 42%, ' + alpha(COLORS.dark, 0.25) + ' 100%)' } }),
      ]
    : [
        h('div', { key: 'base', style: { position: 'absolute', top: 0, left: 0, width: 1200, height: 630, background: COLORS.dark } }),
        h('div', { key: 'glow', style: { position: 'absolute', top: -140, right: -120, width: 720, height: 720, background: 'radial-gradient(circle, ' + alpha(COLORS.blue, 0.55) + ' 0%, ' + alpha(COLORS.blue, 0) + ' 70%)' } }),
        h('div', { key: 'glow2', style: { position: 'absolute', bottom: -220, left: -160, width: 640, height: 640, background: 'radial-gradient(circle, ' + alpha(COLORS.mint, 0.16) + ' 0%, ' + alpha(COLORS.mint, 0) + ' 70%)' } }),
      ]
  ...
  const content = [
    h('div', { key: 'eyebrow', style: { display: 'flex', fontSize: 20, fontWeight: 500, color: COLORS.mint, marginBottom: 22 } }, card.eyebrow),
  ...
  return h('div', { style: { width: '100%', height: '100%', display: 'flex', position: 'relative', fontFamily: 'Manrope' } }, [
    ...background,
...
async function renderCardDef(card) {
  ...
  // Satori (behind ImageResponse) parses neither woff2 nor variable fonts: a
  // variable TTF throws in parseFvarAxis. So OG rendering uses static TTF
  // instances while the browser gets the variable woff2 subsets. Same typeface.
  const manropeRegular = readFileSync(join(root(), 'public/fonts/manrope-og-400.ttf'))
  const manropeMedium = readFileSync(join(root(), 'public/fonts/manrope-og-500.ttf'))
  let photoSrc = null
  if (card.photo) {
