...
import { getMeetingsForCompany, type AdminMeetingRow } from "@/entities/company-os";
...
export async function listProgramSummaries(
  ...
  return programs.map((p) => {
    ...
    return {
      ...
      aiTokens: ai,
      leverage: leverageOf(ai, repo ? humanByRepo.get(repo.id) ?? 0 : 0),
...
async function fetchPrPage(repoId: string, opts: ProgramPrOptions): Promise<PrPage> {
  ...
  let rowQb = htt.from("pull_requests").select("id, number, title, state, author_login, url, merged_at, opened_at").eq("repo_id", repoId);
  if (term) {
    const pat = `%${term}%`;
    const ors = [`title.ilike.${pat}`, `author_login.ilike.${pat}`];
    if (/^\d+$/.test(term)) ors.push(`number.eq.${term}`);
