...
import { getWorkboard, getMeetingsForCompany, getInvoicesForCompany, getAssignmentsForCompany, type WorkboardData, type AdminMeetingRow } from "@/entities/company-os";
import type { HubTeam } from "@/entities/company-os";
...
async function groupsForCompanies(companyIds: string[]): Promise<RoadmapGroup[]> {
  ...
  const { data, error: groupsError } = await companyOs
...
export async function getActorClientCompanies(actor: TeamActor): Promise<ClientCompany[]> {
  const { data, error: assignError } = await companyOs
...
export async function hasClientAssignments(actor: TeamActor): Promise<boolean> {
  const { data, error: anyAssignError } = await companyOs.from("staff_assignments").select("id").eq("team_member_id", actor.teamMemberId).eq("status", "active").limit(1);
...
export async function companyHasPrograms(companyId: string): Promise<boolean> {
  const { data, error: programsError } = await companyOs
    .from("ai_programs")
    .select("id")
    .eq("company_id", companyId)
    .limit(1);
...
// HubTeam describes company_os rows and lives in company-os so the admin
// company 360, a layer below /team, can build and render it without importing up.
export type { HubTeam } from "@/entities/company-os";
...
async function programBelongsToCompany(companyId: string, programId: string): Promise<boolean> {
  const { data, error: programError } = await companyOs
    .from("ai_programs")
    .select("id")
    .eq("id", programId)
    .eq("company_id", companyId)
