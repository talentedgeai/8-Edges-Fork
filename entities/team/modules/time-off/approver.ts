...
async function person(id: string | null): Promise<PersonRow | null> {
  ...
  const { data, error } = await companyOs
    ...
    .eq("id", id)
    .maybeSingle();
  // A failed lookup reads as "no approver", which is the same shape as a
  // genuinely missing person. Log it so the two are distinguishable in the
  // server log; the caller contract is unchanged.
  if (error) console.error("[team/time-off] approver person lookup", error);
...
async function clientManagerFor(
  ...
  const { data, error } = await companyOs
    ...
    .limit(1);
  if (error) console.error("[team/time-off] client manager placement", error);
...
export async function resolveLeaveApprover(teamMemberId: string): Promise<LeaveApprover | null> {
  ...
  const { data: member, error: memberError } = await companyOs
    ...
    .eq("id", teamMemberId)
    .maybeSingle();
  if (memberError) console.error("[team/time-off] member manager lookup", memberError);
...
export async function clientWatcherEmails(approver: LeaveApprover): Promise<string[]> {
  ...
  const { data, error } = await companyOs
    ...
    .eq("status", "active")
    .eq("role", "admin");
  if (error) console.error("[team/time-off] client portal admins", error);
  ...
  const { data: people, error: peopleError } = await companyOs
    ...
    .select("email")
    .in("id", ids);
  if (peopleError) console.error("[team/time-off] watcher emails", peopleError);
...
export async function teamMemberIdsManagedBy(
  ...
  if (error) console.error("[team/time-off] members managed by", error);
