...
export async function runReviewScheduler(
  ...
  const { data: memberData, error: memberError } = await companyOs
    .from("team_members")
  ...
  if (memberError) console.error("[review-scheduler] team_members", memberError);
  ...
  const { data: probationRows, error: probationError } = await companyOs
    .from("performance_reviews")
    .select("team_member_id")
    .eq("review_type", "probation");
  if (probationError) console.error("[review-scheduler] performance_reviews", probationError);
  ...
  if (managerIds.length) {
    const { data, error: managerError } = await companyOs
      .from("team_members")
      .select("id, people!person_id(full_name, first_name, preferred_name, email)")
      .in("id", managerIds);
    if (managerError) console.error("[review-scheduler] team_members", managerError);
  ...
  const { data: pendingData, error: pendingError } = await companyOs
    .from("performance_reviews")
    .select("id, team_member_id, reviewer_id, review_type, rater_kind, created_at, reviewer_email, access_token")
    .eq("source", "portal")
    .in("status", ["open", "draft"]);
  if (pendingError) console.error("[review-scheduler] performance_reviews", pendingError);
  ...
  if (recipientTmIds.length) {
    const { data, error: recipientError } = await companyOs
      .from("team_members")
      .select("id, people!person_id(full_name, first_name, preferred_name, email)")
      .in("id", recipientTmIds);
    if (recipientError) console.error("[review-scheduler] team_members", recipientError);
    for (const r of (data ?? []) as Array<{ id: string; people: NameEmail | NameEmail[] | null }>) {
      const p = Array.isArray(r.people) ? r.people[0] ?? null : r.people;
