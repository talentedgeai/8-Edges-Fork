...
import { DENIED, SUBJECT_COMMITMENT } from "@/entities/company-os";
...
describe("moveCard", () => {
  ...
  it("gives a genuine miss the same answer a denied actor gets", async () => {
    ...
    const { moveCard } = await import("./move-card");
    const missing = await moveCard("task-1", "col-b", "board");

    calls.length = 0;
    scripts.clear();
    script("tasks", { data: TASK });
    // Through the door, not the module file: entities/team may only reach
    // company-os by its index. The barrel re-exports the mocked binding.
    const { boardActorFor } = await import("@/entities/company-os");
    vi.mocked(boardActorFor).mockResolvedValue(null);
    const denied = await moveCard("task-1", "col-b", "board");
