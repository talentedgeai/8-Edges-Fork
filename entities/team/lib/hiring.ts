...
import { getLoopsForRequisitions, type LoopStep, recommendationFromDb, type RecommendationKey } from "@/entities/company-os";
import {
  SAIGON_TZ,
  saigonDateKey,
  roundLabel,
...
export async function getTeamHiring(actor: TeamActor): Promise<TeamHiring> {
  ...
  if (appIds.length > 0) {
    const { data: ivRows, error: ivError } = await companyOs
      .from("interviews")
      .select(
...
export async function getMyInterviewDay(actor: TeamActor): Promise<MyInterview[]> {
  const { data: seatRows, error: seatError } = await companyOs
  ...
  const { data: scRows, error: scorecardError } = await companyOs
