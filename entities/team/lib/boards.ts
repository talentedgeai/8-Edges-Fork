...
import { selectTasks, selectBoardMembers, selectBoards, isBoardMember, getBoardBySlug } from "@/entities/boards";
import { getWorkboard, type BoardDetail, type TaskPriority, type WorkboardData } from "@/entities/boards";
...
export async function getMyRecentTasks(actor: TeamActor, limit: number): Promise<RecentTask[]> {
  const { data: taskData, error: tasksError } = await selectTasks("id, title, priority, due_date, board_id, created_at")
  ...
  const { data: boardRows, error: boardsError } = await selectBoards("id, slug, name").in("id", boardIds);
...
export async function getTeamWorkboard(actor: TeamActor): Promise<WorkboardData> {
  ...
  const [memberRes, assignedRes] = await Promise.all([
    selectBoardMembers("board_id").eq("person_id", actor.personId),
    selectStaffAssignments("company_id").eq("team_member_id", actor.teamMemberId).eq("status", "active"),
  ]);
  ...
  if (companyIds.length > 0) {
    const { data, error } = await selectBoards("id")
