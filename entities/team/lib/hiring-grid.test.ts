...
// hiring-grid reaches company-os through its front door, as the boundary rules
...
vi.mock("@/entities/company-os", async () => {
  const panel = await vi.importActual<
    typeof import("@/entities/company-os/modules/hiring/interview-panel")
  >("@/entities/company-os/modules/hiring/interview-panel");
...
describe("buildInterviewIndex", () => {
  ...
  it("counts only submitted human scorecards and averages their scores", () => {
    const idx = buildInterviewIndex(
      [
        rawRow({
          interview_interviewers: [
            { interviewer_id: "h1", people: { email: "h1@edge8.ai", metadata: {} } },
            { interviewer_id: "h2", people: { email: "h2@edge8.ai", metadata: {} } },
          ],
          interview_scorecards: [
  ...
  it("stays blind while a panelist viewer has not filed their own scorecard", () => {
    const seats = [{ interviewer_id: "me", people: { email: "me@edge8.ai", metadata: {} } }];
