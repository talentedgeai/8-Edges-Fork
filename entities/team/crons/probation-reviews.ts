...
import { getProbationRows } from "@/entities/org";
 ...
 * @generator
 */
// 07:00 in Vietnam (GMT+7); Vercel runs cron schedules in UTC.
...
// Where a probation review lands when no manager is resolved. Configured, not
// named: a hardcoded address follows the fork and mails the previous owner.
...
async function handler(req: Request) {
  ...
  for (const r of due) {
    ...
    const link = `${process.env.NEXT_PUBLIC_SITE_URL ?? ""}/admin/talent/team/${r.teamMemberId}`;
