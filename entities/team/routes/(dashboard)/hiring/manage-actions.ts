...
import { advanceApplicationStage, rejectApplicationStage, stampBookingRequested, type PipelineResult } from "@/entities/company-os";
...
async function actorManagesApplication(actor: TeamActor, applicationId: string): Promise<boolean> {
  const { data, error: appError } = await companyOs
    .from("applications")
