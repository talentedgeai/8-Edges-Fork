...
export default async function TeamHome() {
  ...
  // Onboarding content: the hire's own journey (plan, checkpoints, this week's
  // checklist), the first-two-days reading, and the certification CTA.
  ...
  // The one line about an upcoming 1-1, when there is something to do about it
  // (L.7). Coaching owns the rule; the hub only asks and draws the answer.
    ...
    ? hubOneOnOnePrompt(
        {
          ...
          weekday: coaching.nextOneOnOneOn
            ? new Date(`${coaching.nextOneOnOneOn}T00:00:00`).toLocaleDateString("en-GB", { weekday: "long" })
            : null,
        },
        saigonToday(),
      )
  ...
  // ── Main column ─────────────────────────────────────────────────────────
  // A new hire's home leads with the plan itself: the journey card, then this
  // week's checklist from the plan, then the reading every plan opens with.
  const onboardingSection = newHire ? (
    <>
      ...
      {onboarding && (
        <HomeOnboardingPlan
          plan={onboarding}
          roleTitle={profile?.positionTitle ?? null}
          managerName={profile?.managerName ?? null}
          nextOneOnOneOn={coaching?.nextOneOnOneOn ?? null}
        />
      )}
      {onboarding?.week && (
        <HomeWeekChecklist
          week={onboarding.week}
          planHref={onboarding.hasPlan ? `/team/onboarding/plan/${onboarding.journeyId}` : null}
          onToggle={toggleMyOnboardingTask}
        />
      )}
      ...
      <div className="u-grid-2-1">
        ...
        <div>
          <HomePerformance
            goals={homeGoals}
            latest={latestRecap}
            nextOn={coaching?.nextOneOnOneOn ?? null}
            hasCoaching={coaching !== null}
            prompt={oneOnOnePrompt}
            emptyNote={
              newHire && homeGoals.length === 0
                ? `Not written yet. Due day 7${fastGoalDue ? `, ${formatDate(fastGoalDue)}` : ""}. Your plan names the key result to link it to.`
                : null
            }
          />
