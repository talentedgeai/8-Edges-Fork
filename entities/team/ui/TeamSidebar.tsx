...
import { filterNav, type NavGroup, type NavSection } from "@/kernel/shell/nav";
...
export function TeamSidebar({
  ...
  const groups: NavGroup[] = filterNav(
    sections,
    capabilitiesOf(role, isCoach, isHiringManager, hasClients, permissions),
    ...
    <>
      ...
      <nav className={`admin-sidebar${navOpen ? " is-open" : ""}`} aria-label="Team">
        <div className="admin-brand">
          <span className="admin-brand-lead">8 Edges Team</span>
        ...
        <div className="admin-nav" onClick={() => setNavOpen(false)}>
          {groups.map((group, gi) => {
            ...
            <div className="admin-nav-group" key={group.label ?? `g${gi}`}>
              ...
              {!isCollapsed && (
                <TeamNavEntries entries={group.items} groupLabel={group.label} isActive={(href) => isActive(pathname, href)} />
              )}
            </div>
            );
          })}
