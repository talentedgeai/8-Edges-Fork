...
vi.mock("@/kernel/messaging/email", () => ({ sendTransactionalEmail }));
vi.mock("@/kernel/config/site-origin", () => ({ getSiteOrigin: () => "https://www.edge8.ai" }));

vi.mock("@/entities/retreats/my-retreat/access", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/entities/retreats/my-retreat/access")>();
...
} from "@/entities/retreats/my-retreat/access";
