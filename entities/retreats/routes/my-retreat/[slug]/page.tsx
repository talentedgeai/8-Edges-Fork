...
async function completedSurveySlugs(cohort: string, personId?: string, email?: string): Promise<Set<string>> {
  ...
  const { data, error: surveyResponsesError } = await companyOs
    .from("survey_responses")
    .select("surveys(slug)")
    .eq("cohort_slug", cohort)
    .or(or.join(","));
  if (surveyResponsesError) console.error("[retreats] survey_responses read failed:", surveyResponsesError.message);
