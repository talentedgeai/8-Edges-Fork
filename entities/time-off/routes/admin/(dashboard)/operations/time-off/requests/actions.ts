...
export async function decideTimeOff(
  ...
  const next = nextLeaveStatus(row.status as LeaveStatus, decision, "admin");
...
export async function cancelTimeOff(id: string): Promise<Result> {
  ...
  const next = nextLeaveStatus(row.status as LeaveStatus, "cancelled", "admin");
  if (next.outcome === "refuse") return { ok: false, error: next.error };
  if (next.outcome === "noop") return { ok: true };

  const { error } = await updateTimeOff({ status: next.status })
    .eq("id", id);
  if (error) return { ok: false, error: error.message };
