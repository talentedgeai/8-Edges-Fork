...
export async function requestOwnTimeOff(input: {
  ...
  // (C.24, product call), stamped like a policy auto-approval — approved_by
  // null — so the board shows it as "auto" rather than as a person's decision.
...
export async function cancelOwnTimeOff(id: string): Promise<Result> {
  ...
  const next = nextLeaveStatus(r.status as LeaveStatus, "cancelled", "employee");
  if (next.outcome === "refuse") return { ok: false, error: next.error };
  if (next.outcome === "noop") return { ok: true };

  const { ok, error } = await teamUpdateInScope(actor, "time_off", id, { status: next.status });
