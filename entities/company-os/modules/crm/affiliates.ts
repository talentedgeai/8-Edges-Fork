...
async function primaryContactsFor(companyIds: string[]): Promise<Map<string, { personId: string; fullName: string | null; email: string }>> {
  ...
  const { data, error: contactsErr } = await companyOs
  ...
  const [{ data: commRows }, { data: dealRows }] = await Promise.all([
    codeIds.length
      ? companyOs
          ...
          .select("id, affiliate_id, source_event, source_ref, gross_cents, rate, commission_cents, redemption_choice, chosen_at, payout_id, notes, created_at, payout:affiliate_payouts!payout_id(method)")
...
export async function generateAffiliateCode(name: string | null, email: string): Promise<string> {
  ...
  const { data, error: codesErr } = await companyOs.from("affiliates").select("code").ilike("code", `${base}%`);
