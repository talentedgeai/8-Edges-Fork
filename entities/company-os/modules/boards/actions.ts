...
import { endPosition, mergeCardMeta, refresh } from "./card-helpers";
import { TASK_PRIORITIES, SUBJECT_COMMITMENT, SUBJECT_BACKLOG_ITEM, EPIC_COLORS, type TaskPriority } from "./types";
...
export async function createCard(raw: CreateCardInput): Promise<Result & { id?: string }> {
  ...
  const { data: col, error: colErr } = await companyOs.from("board_columns").select("id, is_done").eq("id", input.columnId).eq("board_id", input.boardId).maybeSingle();
...
export async function setCardRoadmapItem(
  ...
  if (backlogItemId) {
    ...
    const { data: item, error: itemErr } = await companyOs.from("client_backlog_items").select("id").eq("id", backlogItemId).eq("company_id", clientId).maybeSingle();
...
export async function updateBoard(
  ...
  if (patch.aiProgramId !== undefined) {
    ...
    if (currentProgramId) {
      ...
      let keep = false;
      if (newClientId) {
        const { data: program, error: keepErr } = await companyOs
          .from("ai_programs")
          .select("company_id")
...
export async function toggleSubtask(subtaskId: string, done: boolean, boardSlug: string): Promise<Result> {
  const gate = await boardMutation({ table: "tasks", id: subtaskId, label: "subtask" });
  if (!gate.ok) return gate;
  const { actor } = gate;
  const { error } = await companyOs
    .from("tasks")
...
export async function setSprintMeeting(sprintId: string, meetingId: string | null, boardSlug: string): Promise<Result> {
  ...
  const { actor } = gate;
  if (meetingId) {
    const { data: m, error: meetingErr } = await companyOs.from("meetings").select("id").eq("id", meetingId).maybeSingle();
...
export async function pullSprintBriefFromMeeting(
  ...
  if (b?.client_company_id) {
    const { data: co, error: coErr } = await companyOs.from("companies").select("name").eq("id", b.client_company_id).maybeSingle();
    if (coErr) console.error("[company-os/boards] companies", coErr);
