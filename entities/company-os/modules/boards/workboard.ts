...
export async function getWorkboard(query: WorkboardQuery): Promise<WorkboardData> {
  ...
  const [columnsRes, membersRes, sprintsRes, epicsRes, tasks, assignedRes, clientRes, programRes, contactsRes] = await Promise.all([
    ...
    clientIds.length
      ? companyOs
          ...
          .select("company_id, team_members!team_member_id(person_id)")
          .in("company_id", clientIds)
          .eq("status", "active")
      : Promise.resolve({ data: [] as unknown[], error: null }),
    clientIds.length
      ? companyOs.from("companies").select("id, name").in("id", clientIds)
      : Promise.resolve({ data: [] as { id: string; name: string }[], error: null }),
    programIds.length
      ? companyOs.from("ai_programs").select("id, name").in("id", programIds)
      : Promise.resolve({ data: [] as { id: string; name: string }[], error: null }),
    ...
    !query.clientSafe && clientIds.length
      ? companyOs.from("person_companies").select("person_id").in("company_id", clientIds)
  ...
  for (const [label, res] of [
    ...
    if (res.error) console.error(`[company-os/boards] ${label}`, res.error);
  }
  ...
  const commitmentIds = parents.filter((t) => t.subject_type === SUBJECT_COMMITMENT && t.subject_id).map((t) => t.subject_id as string);
  ...
  const [peopleRes, commitmentsRes, backlogLabelRes, clientBacklogRes, roadmapGroupsRes, logsRes, commentsRes, archivedRes] =
    await Promise.all([
      personIds.length
      ...
      backlogIds.length
        ...
        : Promise.resolve({ data: [] as { id: string; title: string }[], error: null }),
      singleClient
        ? companyOs.from("client_backlog_items").select("id, title, group_key").eq("company_id", singleClient).is("archived_at", null).order("sort_order")
        : Promise.resolve({ data: [] as BacklogRef[], error: null }),
      singleClient
        ? companyOs.from("client_roadmap_groups").select("key, step_label, title, sort_order").eq("company_id", singleClient).is("archived_at", null).order("sort_order")
  ...
  for (const [label, res] of [
    ["people", peopleRes],
    ["coaching_commitments", commitmentsRes],
