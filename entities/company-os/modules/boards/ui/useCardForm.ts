...
import { SUBJECT_BACKLOG_ITEM, SUBJECT_COMMITMENT, cardBuildSummary, cardPrUrl } from "@/entities/company-os/modules/boards/types";
import {
  archiveCard,
  createCard,
  setCardEpic,
  ...
  setCardSprint,
  updateCard,
} from "@/entities/company-os/modules/boards/actions";
...
export function useCardForm({
  ...
  function openCreate(laneId: string) {
    const preset = single && sprintFilter !== "all" && sprintFilter !== "backlog" ? sprintFilter : "";
    const epicPreset = single && epicFilter !== "all" && epicFilter !== "none" ? epicFilter : "";
    setForm({
      id: null,
      boardId: single?.id ?? "",
      clientId: single ? single.client_company_id ?? INTERNAL : "",
      laneId,
      title: "",
      priority: "p3",
      assigneeId: "",
  ...
  function save() {
    ...
    if (!board) return setBanner("Pick the board the card belongs to.");
