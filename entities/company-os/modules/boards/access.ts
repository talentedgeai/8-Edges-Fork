...
export async function isBoardMember(boardId: string, personId: string, teamMemberId: string): Promise<boolean> {
  ...
  if (memRes.error) console.error("[company-os/boards] board_members", memRes.error);
  if (boardRes.error) console.error("[company-os/boards] boards", boardRes.error);
  if (memRes.data) return true;
  const companyId = (boardRes.data as { client_company_id: string | null } | null)?.client_company_id;
  if (!companyId) return false;
  const { data, error: assignmentErr } = await companyOs
    ...
    .limit(1)
    .maybeSingle();
  if (assignmentErr) console.error("[company-os/boards] staff_assignments", assignmentErr);
  return Boolean(data);
}
