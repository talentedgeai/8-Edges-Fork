...
export async function listBoards(): Promise<BoardListItem[]> {
  ...
  if (boardsErr) console.error("[company-os/boards] boards", boardsErr);
...
export async function listRecentMeetings(limit = 40): Promise<MeetingOption[]> {
  const { data, error: meetingsErr } = await companyOs
    .from("meetings").select("id, title, started_at").is("archived_at", null)
    .not("started_at", "is", null).order("started_at", { ascending: false }).limit(limit);
  if (meetingsErr) console.error("[company-os/boards] meetings", meetingsErr);
...
export async function listActiveBoards(): Promise<{ id: string; slug: string; name: string }[]> {
  const { data, error: activeErr } = await companyOs
    ...
    .is("archived_at", null)
    .order("sort_order");
  if (activeErr) console.error("[company-os/boards] boards", activeErr);
