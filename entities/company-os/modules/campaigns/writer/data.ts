...
export async function loadCampaign(id: string): Promise<Loaded<WriterCampaign>> {
  const { data, error } = await companyOs
    ...
    .select("id, name, idea, objective, brand_id, pillar_id, starts_on, writer_step, writer_started_at, writer_error")
    .eq("id", id)
    .maybeSingle();
...
export async function loadBlogAsset(campaignId: string): Promise<Loaded<BlogAsset>> {
  const { data, error } = await companyOs
    ...
    .select("id, title, copy_md, seo_md, image_url, image_brief_md, notes, status, posted_url")
...
export async function listCampaignAssets(campaignId: string): Promise<Loaded<CampaignAsset[]>> {
  const { data, error } = await companyOs
    .from("marketing_content")
    .select("id, channel, status, image_url")
    .eq("campaign_id", campaignId)
    .neq("status", "skipped");
  if (error) return { ok: false, error: error.message };
