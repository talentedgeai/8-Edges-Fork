...
import type { BroadcastBlocks, RenderedBlocks } from "@/entities/site";
import { siteForBrandSlug } from "@/entities/company-os/modules/campaigns/brand-sites";
...
export async function resolveBroadcastBlocks(blocks: BroadcastBlocks): Promise<RenderedBlocks> {
  ...
  const { data, error } = await companyOs
    ...
    .select("id, title, excerpt, image_url, slug, status, brands(slug), marketing_pillars(name)")
    .in("id", blocks.posts);
  if (error) {
