...
import { parseBroadcastBlocks, type BroadcastBlocks } from "@/entities/site/client"; // browser door: reached from a client component via marketing-calendar.ts
...
const HOME_BRAND_SLUG = "arca-wellness";
...
export async function listRecipients(broadcastId: string, limit = 200): Promise<RecipientRow[]> {
  const { data, error: recipientsErr } = await companyOs
