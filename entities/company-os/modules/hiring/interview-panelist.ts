...
import { AI_PANELIST_EMAIL, AI_PANELIST_NAME, DEFAULT_CRITERIA, recommendationToDb } from "@/entities/company-os/modules/hiring/interview-panel";
...
async function priorRounds(applicationId: string, createdAt: string): Promise<string> {
  const { data, error: priorErr } = await companyOs
    .from("interviews")
