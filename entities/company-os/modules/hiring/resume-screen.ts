...
import { setCandidateAiSalary } from "@/entities/company-os/modules/hiring/candidate-sensitive";
...
// The runtime mirror of SCREEN_SCHEMA. Structured output is a request, not a
// guarantee, so the model's JSON is parsed through this before it is written.
const screenOutput = z.object({
  overview: z.string(),
...
const SCREEN_SYSTEM = `You are the recruiting screener for Arca Wellness, an AI consulting and staffing company in Vietnam. You review one job application at a time against its job requisition and produce a structured screen: a summary following Arca Wellness's template, plus a 0-5 fit rating used to stack-rank all applicants for the role.
...
async function markFailed(applicationId: string, error: string): Promise<Err> {
  ...
  const { error: stampErr } = await companyOs
    .from("applications")
    .update({
...
async function runScreen(applicationId: string): Promise<Ok | Err> {
  ...
  // validate before any of it reaches the application row.
  let rawOut: unknown;
  try {
    rawOut = JSON.parse(out.text);
  } catch (e) {
    return markFailed(applicationId, `The model returned invalid JSON: ${e instanceof Error ? e.message : String(e)}`);
  }
  const validated = screenOutput.safeParse(rawOut);
  if (!validated.success) return markFailed(applicationId, zodIssuesToMessage(validated.error.issues));
