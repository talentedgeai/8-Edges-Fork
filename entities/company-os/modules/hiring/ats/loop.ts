...
export async function getLoopsForRequisitions(
  ...
  const { data: stepRows, error: stepErr } = await companyOs
...
async function nextPosition(reqId: string): Promise<number> {
  const { data, error: posErr } = await companyOs
    .from("requisition_loop_steps")
    .select("position")
...
export async function moveLoopStep(stepId: string, direction: "up" | "down"): Promise<Result> {
  const { data, error: stepReadErr } = await companyOs
    .from("requisition_loop_steps")
  ...
  const { data: neighbourRow, error: neighbourErr } = await companyOs
    .from("requisition_loop_steps")
    .select("id, position")
