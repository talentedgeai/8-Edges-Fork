...
export default async function ContractorPaymentsPage({ searchParams }: { searchParams: SearchParamsObj }) {
  ...
  // salaries (the finance-cleared admins): non-cleared admins get a notice and no query runs.
  ...
  if (!viewer?.canViewSensitive) {
    return (
      <>
        ...
        />
        <p className="admin-cell-muted">Restricted — visible to the finance-cleared admins only.</p>
      </>
    );
  }
