...
import { stripPostgrestMetacharacters } from "../deals/board-helpers";
...
export async function setInvoiceCustomerCompany(invoiceId: string, companyId: string | null): Promise<MapResult> {
  ...
  const { data: invoice, error: invErr } = await companyOs
    .from("invoices")
    .select("entity, customer_id, external_id")
    .eq("id", invoiceId)
    .maybeSingle();
  ...
  if (!customerId) {
    ...
    await companyOs.from("invoices").update({ customer_id: customerId }).eq("id", invoiceId);
  }
  ...
  for (const h of (holders ?? []) as { id: string; metadata: Record<string, unknown> | null }[]) {
    ...
    const { error } = await companyOs.from("companies").update({ metadata: { ...md, [key]: next } as Json }).eq("id", h.id);
  ...
  if (companyId) {
    ...
    if (!ids.includes(customerId)) {
      const { error } = await companyOs
        .from("companies")
        .update({ metadata: { ...md, [key]: [...ids, customerId] } as Json })
        .eq("id", companyId);
      if (error) return { ok: false, error: error.message };
    }
  ...
  const { data: touched, error: backfillErr } = await companyOs
    .from("invoices")
