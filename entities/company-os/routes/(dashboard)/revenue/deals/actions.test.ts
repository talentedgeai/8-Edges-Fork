...
vi.mock("@/entities/company-os/lib/mutations", () => ({
  archiveRecord: vi.fn(),
  guardedDelete: vi.fn(),
  restoreRecord: vi.fn(),
}));
...
vi.mock("@/entities/company-os/lib/lifecycle", () => ({
...
vi.mock("@/entities/company-os/lib/fx", () => ({
...
import { normalizeUrl, stripPostgrestMetacharacters } from "./board-helpers";
...
describe("normalizeUrl", () => {
  it("keeps a URL that already has a scheme, in either case", () => {
    expect(normalizeUrl("https://edge8.ai/x")).toBe("https://edge8.ai/x");
    expect(normalizeUrl("http://edge8.ai")).toBe("http://edge8.ai");
    expect(normalizeUrl("HTTPS://edge8.ai")).toBe("HTTPS://edge8.ai");
  });
  ...
  it("trims before deciding", () => {
    expect(normalizeUrl("  edge8.ai  ")).toBe("https://edge8.ai");
    expect(normalizeUrl(" https://edge8.ai ")).toBe("https://edge8.ai");
  });
