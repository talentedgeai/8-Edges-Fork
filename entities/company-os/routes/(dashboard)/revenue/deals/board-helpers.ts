...
// ─── Shared with the deal server actions ─────────────────────────────────────
// These two live here rather than in actions.ts because that file is
// "use server": every export there must be an async server action, so a plain
// helper has nowhere to sit — and no way to be covered on its own.

/** A user-typed URL, given a scheme when it lacks one. Empty becomes null so
 *  the column clears rather than storing "". */
