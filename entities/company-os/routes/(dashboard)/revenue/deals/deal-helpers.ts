...
import { getLead, recordTransition } from "@/entities/company-os/lib/lifecycle";
...
export async function applyUsdConversion(
  ...
  try {
    ...
    if (opts.cacheRate) {
      ...
      const { error: fxErr } = await companyOs
        .from("fx_rates")
        .upsert({ currency: currency.toLowerCase(), rate_to_usd: fx.rate, updated_at: new Date().toISOString() }, { onConflict: "currency" });
