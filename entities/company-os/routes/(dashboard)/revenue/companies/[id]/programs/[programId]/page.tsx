...
import { listHoursLedger, listFocusBudgets } from "@/entities/htt";
import { HoursLedger } from "@/entities/htt/client";
import { adminOverrideDayHours, adminClearDayHours, adminSetFocusBudget, adminRescanHours } from "@/entities/company-os/modules/crm/hours-actions";
import { getLiveCardItemIds, listAssignableStaff } from "@/entities/company-os/modules/crm/company-hub";
...
export default async function ProgramDetailPage({
  ...
  const [liveCardItemIds, boardDetail, boardOptions, admin, assignees, ledger] = await Promise.all([
    ...
    detail.repoId ? listHoursLedger(detail.repoId) : Promise.resolve([]),
  ]);
  const budgets = await listFocusBudgets([...new Set(ledger.map((r) => r.personId).filter((p): p is string => !!p))]);
            ...
            <>
              ...
              <div className="u-mt-4">
                <HoursLedger
                  ...
                  viewerPersonId={viewerPersonId}
                  canEditAll
                  overrideAction={adminOverrideDayHours}
                  clearAction={adminClearDayHours}
                  budgetAction={adminSetFocusBudget}
                  rescanAction={adminRescanHours}
                />
              </div>
            </>
          )}
