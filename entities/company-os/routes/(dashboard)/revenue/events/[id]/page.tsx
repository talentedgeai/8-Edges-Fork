...
import { listAssignablePeople, listPeopleNames } from "@/entities/company-os/modules/crm/people-options";
...
import { getEventPnlLines } from "@/entities/company-os/lib/event-pnl";
import { PnlTab } from "./PnlTab";
import { getEventAgenda } from "@/entities/company-os/lib/event-agenda";
...
export default async function EventDetailPage({ params }: { params: { id: string } }) {
  ...
  if (event.feedback_survey_id) {
    const { data: survey, error: surveyError } = await companyOs.from("surveys").select("slug").eq("id", event.feedback_survey_id).maybeSingle();
  ...
  // tallied per survey.
  const { data: fbRows, error: fbError } = await companyOs.from("survey_responses").select("submitted_at, surveys(id, slug, name)").eq("cohort_slug", event.slug);
