...
import { qrPngDataUrl } from "@/entities/retreats";
...
export async function updateEvent(eventId: string, patch: EventPatch): Promise<Result> {
  ...
  if (patch.feedback_survey_id !== undefined) {
    if (patch.feedback_survey_id !== null) {
      const { data: survey, error: surveyErr } = await companyOs.from("surveys").select("id").eq("id", patch.feedback_survey_id).maybeSingle();
      if (surveyErr) return { ok: false, error: surveyErr.message };
...
export async function getEventQrs(
  ...
  try {
    ...
    if (event.feedback_survey_id) {
      ...
      const { data: survey, error: surveyErr } = await companyOs.from("surveys").select("slug").eq("id", event.feedback_survey_id).maybeSingle();
