...
export function RegenerateModal({
  ...
  // The prompt is fetched once when the modal opens. The parent passes an inline
  // closure, so depending on it directly would refetch on every parent render.
  const loadPromptRef = useRef(loadPrompt);
  loadPromptRef.current = loadPrompt;
  useEffect(() => {
    let live = true;
    loadPromptRef.current().then((r) => {
      if (!live) return;
