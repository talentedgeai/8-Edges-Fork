...
import { insertContractorWorkRequests, updateContractorWorkRequests } from "@/entities/portal";
...
export async function sendWorkRequest(id: string): Promise<Result> {
  ...
  if (req.status === "draft") {
    const { error } = await updateContractorWorkRequests({ status: "awaiting_estimate", updated_at: new Date().toISOString() })
      .eq("id", id);
    if (error) return { ok: false, error: error.message };
  }
