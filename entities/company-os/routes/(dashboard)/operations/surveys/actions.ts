...
export async function deleteSurvey(id: string): Promise<Result> {
  ...
  // Read for the audit trail only; a failed read must not block the delete, but
  // it should not be silent either.
  const { data: survey, error: surveyErr } = await companyOs
    .from("surveys")
    .select("slug, name")
    .eq("id", id)
    .maybeSingle();
  if (surveyErr) {
    console.error(`[operations/surveys] survey lookup ${id} failed:`, surveyErr.message);
  }
...
export async function updateField(fieldId: string, input: FieldInput): Promise<Result> {
  ...
  const patch: CompanyOsUpdate<"survey_fields"> = {
    label,
    help_text: input.helpText.trim() || null,
    required: input.required,
    updated_at: new Date().toISOString(),
