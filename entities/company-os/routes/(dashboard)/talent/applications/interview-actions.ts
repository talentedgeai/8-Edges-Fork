...
import { ensureAiPanelist, scoreInterview } from "@/entities/company-os/modules/hiring/interview-panelist";
import { writeScorecard, type ScorecardInput } from "@/entities/company-os/modules/hiring/ats/scorecard";
import {
  ROUND_MODES,
  recommendationFromDb,
  isAiPanelist,
  type RecommendationKey,
} from "@/entities/company-os/modules/hiring/interview-panel";
...
export async function deleteInterviewRound(roundId: string): Promise<Result> {
  const admin = await requireSuperAdmin();
  const { error: docsErr } = await companyOs.from("documents").delete().eq("entity_type", "interview").eq("entity_id", roundId);
