...
export type { PersonOption } from "@/entities/company-os/modules/crm/people-options";
...
export { listAssignablePeople } from "@/entities/company-os/modules/crm/people-options";

export async function listVendorOptions(): Promise<VendorOption[]> {
  const { data, error: vendorsErr } = await companyOs
    .from("vendors")
    .select("id, name")
    .is("archived_at", null)
