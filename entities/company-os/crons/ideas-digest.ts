...
async function handler(req: Request) {
  ...
  // The origin comes from the environment (kernel/config/site-origin) so a
  // preview deployment links into itself rather than production.
  const backlogUrl = `${getSiteOrigin()}/admin/innovation/ideas`;
  ...
  const html =
    ...
    `<p><a href="${backlogUrl}">Open the idea backlog</a></p>`;
