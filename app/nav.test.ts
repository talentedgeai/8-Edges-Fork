...
describeEdge8("the team hub sidebar", () => {
  it("shows a plain member no My Team group at all", () => {
    ...
    expect(nav["My Work"]).toEqual(["Workboard", "Sprint planning", "Time Off"]);
  });
  ...
  it("gives every viewer the same Me and Company rows, which nothing gates", () => {
    const me = ["My Coach", "Reviews", "Ideas", "Profile", "My Equipment"];
    for (const v of [PLAIN, { role: "manager" } as Viewer, { role: "employee", coach: true } as Viewer]) {
      ...
      expect(sidebarOf(v)["Company"]).toContain("Vendors");
    }
  });
  ...
  it("shows the Revenue group only to a member holding the revenue permission", () => {
    expect(sidebarOf(PLAIN)["Revenue"]).toBeUndefined();
    const held = capabilitiesOf("employee", false, false, false, ["revenue"]);
    expect(held).toEqual(["revenue"]);
    const revenue = filterNav(TEAM_NAV as NavSection[], held).flatMap((s) => s.groups).find((g) => g.label === "Revenue");
    ...
    expect(revenue?.items.map((i) => (isSubsection(i) ? `${i.subheading}: ${i.items.map((x) => x.label).join(", ")}` : i.label))).toEqual([
      "CRM: Cockpit, Deals, Leads, Inquiries, Companies, Clients, Meeting Notes, Sales Intelligence",
