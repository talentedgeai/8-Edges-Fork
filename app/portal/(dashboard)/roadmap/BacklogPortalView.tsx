...
export function BacklogPortalView({
  ...
  function renderItem(it: BacklogItem, index: number) {
    ...
    return (
      <Draggable draggableId={it.id} index={index} key={it.id} isDragDisabled={!canPrioritize}>
        {(provided, snapshot) => (
          <div
            className={`admin-backlog-portal-item${snapshot.isDragging ? " dragging" : ""}`}
            ref={provided.innerRef}
            {...provided.draggableProps}
          >
            <span
              className="admin-backlog-portal-handle"
            ...
            <div className="admin-backlog-portal-main">
              <div className="admin-backlog-portal-item-top">
                {it.ref && <span className="admin-backlog-portal-ref">{it.ref}</span>}
                <span className="admin-backlog-portal-title">{it.title}</span>
                <span className="admin-backlog-portal-pills">
                  {canPrioritize ? (
                    BACKLOG_PRIORITIES.map((p) => (
                      <button
                        key={p}
                        type="button"
                        className={`admin-backlog-portal-pill${eff === p ? ` on-${p}` : ""}`}
                        disabled={pending}
                    ...
                    <span className={`admin-backlog-portal-pill admin-backlog-portal-pill--static on-${eff}`}>{PRIORITY_LABEL[eff]}</span>
                  )}
                </span>
              </div>
              <div className="admin-backlog-portal-body">
                ...
                <div className="admin-backlog-portal-chips">
                  {(it.needs ?? []).map((n) => <span key={n} className="admin-backlog-portal-chip">{n}</span>)}
                  {tok && <span className="admin-backlog-portal-chip tok">est. {tok} Human Tokens</span>}
                  {it.source === "client" && (
                    <span className="admin-backlog-portal-chip proposed">{it.status === "proposed" ? "your proposal, awaiting Edge8" : "your idea"}</span>
                  )}
                  {it.client_priority && it.client_priority !== it.edge8_priority && (
                    <span className="admin-backlog-portal-chip mine">you changed from Edge8&apos;s {PRIORITY_LABEL[it.edge8_priority]}</span>
                  )}
                </div>
                {canPrioritize && it.client_priority && (
                  <div className="admin-backlog-portal-hint">
  ...
  return (
    <div className="admin-backlog-portal">
      <p className="admin-backlog-portal-intro">
      ...
      <div className="admin-backlog-portal-counts">
        {BACKLOG_PRIORITIES.map((p) => (
          <span key={p} className="admin-backlog-portal-count">{PRIORITY_LABEL[p]}: {counts[p]}</span>
        ))}
      </div>

      {err && <div className="admin-backlog-portal-err">{err}</div>}

      <DragDropContext onDragEnd={onDragEnd}>
        {groups.map((g) => {
          const groupItems = ordered.filter((i) => i.group_key === g.key);
          return (
            <div key={g.key} className="admin-backlog-portal-group">
              <div className="admin-backlog-portal-group-head">
                {g.step_label && <span className="admin-backlog-portal-step">{g.step_label}</span>}
                <span className="admin-backlog-portal-group-title">{g.title}</span>
              </div>
              {g.intro && <div className="admin-backlog-portal-group-intro">{g.intro}</div>}
              ...
              {companyId && canPropose && (proposeGroup === g.key ? (
                <div className="admin-backlog-portal-propose">
                  <ProposeAssist
                    onDraft={(d) => {
                      setPTitle(d.title);
                      setPNote(d.note);
                  ...
                  {pHint && <div className="admin-backlog-portal-hint u-mb-2">{pHint}</div>}
                  <div className="u-row">
                    <button
                      type="button"
                      className="admin-backlog-portal-btn"
                    ...
                    <button type="button" className="admin-backlog-portal-btn ghost" onClick={() => { setProposeGroup(null); setPTitle(""); setPNote(""); setPHint(null); }}>Cancel</button>
                  </div>
                </div>
              ) : (
                <button type="button" className="admin-backlog-portal-link u-mt-2" onClick={() => setProposeGroup(g.key)}>
