// GENERATED by scripts/gen-app-mounts.mjs — do not edit.
// The body lives in entities/retreats/routes/team/(dashboard)/revenue/events/[id]/roster.csv/route; this file
// exists because Next binds a route by the exports of the file under app/.
// Anything below that a re-export cannot carry is declared by the entity, in
// its mounts.ts.
export { GET } from "@/entities/retreats/routes/team/(dashboard)/revenue/events/[id]/roster.csv/route";

export const dynamic = "force-dynamic";
