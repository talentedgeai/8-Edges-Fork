...
export async function GET(req: Request) {
  ...
  for (const person of byPerson.values()) {
    ...
    const items = person.cards
      .map((c) => {
        const due = c.overdue
          ? `<span style="color:#b0332f;font-weight:600">${escapeHtml(fmtDue(c.due))} (overdue)</span>`
          : `<span style="color:#797c82">${escapeHtml(fmtDue(c.due))}</span>`;
        return `<li style="margin:0 0 8px"><a href="${origin}/team/boards/${escapeHtml(c.slug)}" style="color:#287BE8;text-decoration:none;font-weight:600">${escapeHtml(
          c.title,
        )}</a><br><span style="font-size:13px;color:#797c82">${escapeHtml(c.board)} · </span>${due}</li>`;
      })
      .join("");
