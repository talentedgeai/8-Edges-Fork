...
const EDGE_STROKE: Record<GraphEdgeKind, { stroke: string; dash?: string }> = {
  ...
  agent: { stroke: 'var(--wf-green)' },
  fail: { stroke: 'var(--wf-red)', dash: '5 4' },
...
function NodeShape({ n }: { n: GraphNode }) {
  ...
  switch (n.kind) {
    ...
    case 'write':
      shape = (
        <rect x={left} y={top} width={w} height={h} rx={10} style={{ fill: 'color-mix(in srgb, var(--color-ok-strong) 8%, transparent)', stroke: 'var(--wf-green)', strokeWidth: 1.5 }} />
      )
      break
    case 'decision': {
      const c = 14
      const pts = [
        ...
        .map((p) => p.join(','))
        .join(' ')
      shape = <polygon points={pts} style={{ fill: 'color-mix(in srgb, var(--color-primary-blue) 6%, transparent)', stroke: 'var(--blue)', strokeWidth: 1.5 }} />
      break
    }
    case 'wait':
      shape = (
        <rect x={left} y={top} width={w} height={h} rx={h / 2} style={{ fill: 'color-mix(in srgb, var(--color-warn-strong) 8%, transparent)', stroke: 'var(--wf-amber)', strokeWidth: 1.5, strokeDasharray: '5 4' }} />
      )
      break
    case 'human':
      shape = (
    ...
    case 'flag':
      shape = (
        <rect x={left} y={top} width={w} height={h} rx={10} style={{ fill: 'color-mix(in srgb, var(--color-err-strong) 6%, transparent)', stroke: 'var(--wf-red)', strokeWidth: 1.5 }} />
      )
      break
    case 'state':
      shape = (
...
const LEGEND_SWATCH: Record<string, CSSProperties> = {
  ...
  write: { background: 'color-mix(in srgb, var(--color-ok-strong) 8%, transparent)', border: '1.5px solid var(--wf-green)' },
  action: { background: 'var(--white)', border: '1.5px solid var(--input-border)' },
  wait: { background: 'color-mix(in srgb, var(--color-warn-strong) 8%, transparent)', border: '1.5px dashed var(--wf-amber)', borderRadius: 9 },
  human: { background: 'var(--white)', border: '2px solid var(--dark)' },
  terminal: { background: 'var(--white)', border: '1.5px solid var(--tint-deep)' },
  flag: { background: 'color-mix(in srgb, var(--color-err-strong) 6%, transparent)', border: '1.5px solid var(--wf-red)' },
