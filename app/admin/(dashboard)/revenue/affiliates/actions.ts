...
async function ensurePortalAllowlist(personId: string, invitedBy: string, companyId?: string | null): Promise<void> {
  ...
  await companyOs.from("portal_members").insert({
    person_id: personId,
    company_id: cid,
...
export async function setCommissionRedemption(commissionId: string, choice: "work_credit" | "cash"): Promise<Result> {
  ...
  const { error } = await companyOs
    .from("affiliate_commissions")
    .update({ redemption_choice: choice, rate, commission_cents: commissionCents, chosen_at: new Date().toISOString() })
    .eq("id", commissionId);
...
export async function sendAffiliateInvite(personId: string): Promise<Result> {
  ...
  if (existingAuth) {
    ...
    const { error: upErr } = await companyOs.from("people").update({ auth_user_id: data.user.id }).eq("id", personId);
