...
vi.mock("@/lib/avatars", () => ({ avatarUrlForAuthUser: async () => null }));
vi.mock("@/lib/admin-chat/privileged", () => ({ isPrivilegedChatUser: () => false }));
vi.mock("@/lib/coaching/data", () => ({ isCoach: async () => false }));
vi.mock("@/lib/team/clients", () => ({ hasClientAssignments: async () => false }));
vi.mock("@/lib/team/hiring", () => ({ isHiringManager: async () => false }));
...
vi.mock("@/lib/portal/team", () => ({ hasAssignedStaff: async () => false }));
vi.mock("@/lib/portal/invoices", () => ({ hasInvoices: async () => false }));
vi.mock("@/lib/portal/roles", () => ({ adminCompanyScope: () => [] }));
vi.mock("@/lib/portal/meetings", () => ({ hasMeetings: async () => false }));
vi.mock("@/lib/portal/boards", () => ({ hasBoard: async () => false }));
vi.mock("@/lib/portal/backlog", () => ({ hasBacklog: async () => false }));
vi.mock("@/components/admin/AdminChatWidget", () => ({ AdminChatWidget: () => null }));
vi.mock("@/components/team/TeamChatWidget", () => ({ TeamChatWidget: () => null }));
// Server actions: the sidebars import them for their sign-out form.
vi.mock("@/app/admin/(dashboard)/actions", () => ({ signOut: async () => {} }));
vi.mock("@/app/team/(dashboard)/actions", () => ({ signOut: async () => {} }));
vi.mock("@/app/portal/(dashboard)/actions", () => ({
  signOut: async () => {},
...
function pagesCallingNotFound(dir: string, out: string[] = []): string[] {
  for (const entry of readdirSync(dir)) {
    ...
    } else if (entry === "page.tsx" && IMPORTS_NOT_FOUND.test(readFileSync(p, "utf8"))) {
      out.push(p);
    }
  }
  return out;
