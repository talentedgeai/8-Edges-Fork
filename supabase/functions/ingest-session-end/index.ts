...
Deno.serve(async (req) => {
  ...
  // Schema-v2 usage components (edge8-telemetry >= 1.3.0). Stored on the CLAUDE row
  // only; NULL when absent (v1 record) or non-numeric. Priced at read time, never here.
  ...
  const usage = {
    model: typeof b.model === 'string' && b.model.length > 0 ? b.model : null,
    input_tokens: usageInt(b.input_tokens),
    output_tokens: usageInt(b.output_tokens),
    cache_write_tokens: usageInt(b.cache_write_tokens),
    cache_write_5m_tokens: usageInt(b.cache_write_5m_tokens),
    cache_write_1h_tokens: usageInt(b.cache_write_1h_tokens),
    cache_read_tokens: usageInt(b.cache_read_tokens),
  };
  // Human rows never carry usage; spread explicit nulls so a mixed batch has one column set.
  const noUsage = Object.fromEntries(Object.keys(usage).map((k) => [k, null]));

  const rows = [];
  if (human > 0) rows.push({ ...common, ...noUsage, kind: 'human', amount: human, session_id: session_id ?? null });
  if (claude > 0) rows.push({ ...common, ...usage, kind: 'claude', amount: claude, session_id: session_id ?? null });
