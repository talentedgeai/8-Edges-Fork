...
export type TeamActor = {
  ...
  isAdmin: boolean;
  // Admin-granted team_members.permissions (see TEAM_PERMISSIONS). Read from the
  // employment row on every request, so revoking one takes effect at once.
  permissions: TeamPermission[];
};

// The vocabulary of team_members.permissions, mirrored by its check constraint.
export const TEAM_PERMISSIONS = ["revenue"] as const;
...
type TeamMembershipLookup = {
  ...
  membership: { id: string; status: string; permissions: string[] };
};
...
const findActiveTeamMembership = perRender(async (authUserId: string): Promise<TeamMembershipLookup | null> => {
  ...
  const rows = (memberships ?? []) as { id: string; status: string; permissions: string[] }[];
...
export const getTeamActor = perRender(async (): Promise<GetActorResult> => {
  ...
  return {
    actor: {
      ...
      directReportIds,
      isAdmin,
      // Only known values survive, so a stray string in the column grants nothing.
      permissions: TEAM_PERMISSIONS.filter((p) => (membership.permissions ?? []).includes(p)),
    },
  };
});
