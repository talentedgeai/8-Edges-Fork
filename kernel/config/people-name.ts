...
// What the /team and /portal session actors show as the signed-in person's
// name. Deliberately NOT personName(): the actor is built from an auth-scoped
// people row that carries first_name and is guaranteed an email, and it has
// never consulted display_name. Kept as its own verb so the two guards cannot
// drift apart, not folded into personName(), which would change what both
// surfaces render.
