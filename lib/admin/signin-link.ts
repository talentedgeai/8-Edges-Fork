...
export async function sendAdminSelfServeSignInLink(rawEmail: string): Promise<void> {
  ...
  const { data, error } = await supabase.auth.admin.generateLink({
    type: "magiclink",
    email,
    options: { redirectTo: `${getSiteOrigin()}/api/auth/callback?next=/admin` },
  });
  const tokenHash = data?.properties?.hashed_token;
  if (error || !tokenHash) return;
  const verifyUrl = `${getSiteOrigin()}/admin/verify?token_hash=${encodeURIComponent(tokenHash)}&type=magiclink`;
  ...
  await sendTransactionalEmail({
    ...
    html: `
      ...
      <p style="font-size:13px;color:#64748b;">The button takes you to a sign-in page. Press "Sign in" there and you're in. If the link expires, you can request a fresh one any time at <a href="${getSiteOrigin()}/admin/login">${getSiteOrigin()}/admin/login</a>.</p>
    `,
