    ...
    if (existing) {
      ...
      const sent = await sendTransactionalEmail({
        ...
        html: `
          ...
          <p style="font-size:13px;color:#64748b;">The button takes you to a sign-in page — press "Sign in" there and you're in. If the link expires, request a fresh one at <a href="${getSiteOrigin()}/portal/login">${getSiteOrigin()}/portal/login</a> or reply to this email.</p>
...
export async function resendPortalLinkCore(
  ...
  const sent = await sendTransactionalEmail({
    ...
    html: `
      ...
      <p style="font-size:13px;color:#64748b;">The button takes you to a sign-in page — press "Sign in" there and you're in. If the link expires, you can request a fresh one any time at <a href="${getSiteOrigin()}/portal/login">${getSiteOrigin()}/portal/login</a>.</p>
    `,
